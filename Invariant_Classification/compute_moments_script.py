...
num_train=200 #how many of each character to use in training(testing) sample
num_test=50

tot_train=num_train*10 #Total number of training(testing) samples
tot_test=num_test*10

Xtrain=ld.load_images('train',num_train)
Xtest=ld.load_images('test',num_test)

#Y=Xtest[6,45,:].reshape(28,28)
#plt.imshow(Y)
...
print("-"*30,"Computing Training Data Moments","-"*30)
train.compute_all()
print("-"*30,"Finished Training Data Moments","-"*30)
print("\n")
test=mf.momentClass(Xtest,dMax)
print("-"*30,"Computing Testing Data Moments","-"*30)
test.compute_all()
print("-"*30,"Finished Training Data Moments","-"*30)
