...
def center_of_mass(input, labels=None, index=None):
    ...
    (inf,)
    """
    normalizer = ndimage.measurements.sum(input, labels, index)
    print('normalizer is ',normalizer)
    grids = numpy.ogrid[[slice(0, i) for i in input.shape]]


    results = [ndimage.measurements.sum(input * grids[dir].astype(float), labels, index) / normalizer
